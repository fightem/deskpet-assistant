...
import configparser
from utils.config.setting import *
from utils.config.config import ConfigGetter
from utils.components.bubble import *

from ui.scheduleUI import TodoApp
from ui.MusicPlayer import MP3Player
from ui.Spider_zano import PostQueryTool
from ui.WeatherUI import weatherWindow
from ui.gupiao import StockAnalyzer
from ui.SchoolNotice import NoticeApp
from ui.FetcherNews import HotBoardApp
...
class App(QWidget):
    ...
    def rightMenu(self):
        ...
        # 娱乐模块
        SchoolMenu = QMenu('SchoolMenu')
        SchoolMenu.setTitle('校园学习')
        SchoolMenu.setIcon(QIcon('./data/icon/collection.png'))
        SchoolMenu.addAction(
            QAction(QIcon('./data/icon/settings.png'), '校园通知', self, triggered=self.SchoolNotice)
        )

        # ---> 新增：论文阅读功能 <---
        SchoolMenu.addAction(
            QAction(QIcon('./data/icon/settings.png'), '论文阅读', self, triggered=self.paper_reader)
        )

        SchoolMenu.addAction(
            QAction(QIcon('./data/icon/schedule.png'), '日程表', self, triggered=self.schedule)
        )

        menu.addMenu(SchoolMenu)



        # 娱乐模块
        LifeMenu = QMenu('LifeMenu')
        LifeMenu.setTitle('生活模块')
        LifeMenu.setIcon(QIcon('./data/icon/collection.png'))
        LifeMenu.addAction(
            QAction(QIcon('./data/icon/settings.png'), '音乐播放', self, triggered=self.music)
        )
        LifeMenu.addAction(
            QAction(QIcon('./data/icon/deviceon.png'), '天气情况', self, triggered=self.weather)
        )
        LifeMenu.addAction(
            QAction(QIcon('./data/icon/settings.png'), '集市吃瓜', self, triggered=self.zano)
        )
        LifeMenu.addAction(
            QAction(QIcon('./data/icon/settings.png'), '股情分析', self, triggered=self.stock)
        )
        LifeMenu.addAction(
            QAction(QIcon('./data/icon/settings.png'), '热点新闻', self, triggered=self.HotBoard)
        )

        menu.addMenu(LifeMenu)
    ...
    def zano(self):
        self.zano_window = PostQueryTool()
        self.zano_window.show()

    def stock(self):
        self.stock_window = StockAnalyzer()
        self.stock_window.show()

    def HotBoard(self):
        self.HotBoard_window = HotBoardApp()
        self.HotBoard_window.show()

    def SchoolNotice(self):
        self.school_window = NoticeApp()
        self.school_window.show()

    def paper_reader(self):
        """
        通过子进程启动基于 PyQt6 的论文助手
        """
        # 1. 点击瞬间给出反馈（调用系统托盘的气泡提示）
        cfg = ConfigGetter()
        tray = QSystemTrayIcon(self)
        tray.setIcon(QIcon(cfg.traypath))
        tray.show()
        tray.showMessage("露米娅", "正在为您翻阅文献，启动AI助手大约需要3秒，请稍候...", QSystemTrayIcon.Information,
                         3000)

        # 2. 原有的启动代码
        python_exe = sys.executable
        target_dir = os.path.join(os.getcwd(), 'ai-paper-assister-main')

        try:
            subprocess.Popen([python_exe, "-m", "PaperCompanion.main"], cwd=target_dir)
