...
import os
import json
import threading  # 🟢 [新增] 导入多线程模块，用于后台语音播报
import zhconv  # 🟢 [新增] 导入简繁转换库，专门治繁体字
...
# 🟢 [新增] 导入我们自己写的语音监听和发声模块 (请确保同级目录下有这两个文件)
from .voice_thread import VoiceAssistantThread
from ..utils.tts_module import speak, stop_speaking
...
class ChatWidget(QWidget):
    ...
    def create_input_area(self):
        ...
        self.voice_button.setFixedHeight(36)
        self.voice_button.setStyleSheet("""
            QPushButton {
                background-color: #F5F7FA; border: 1px solid #CFD8DC; border-radius: 8px; padding: 8px 15px; font-weight: bold; color: #555;
            }
            QPushButton:checked {
                background-color: #ffebee; border: 1px solid #ef5350; color: #d32f2f;
            }
        """)
        ...
        control_layout.addWidget(self.voice_button)  # 🟢 [新增] 将语音按钮添加到布局中
    ...
    def toggle_voice_mode(self):
        is_on = self.voice_thread.toggle_mode()
        if is_on:
            self.voice_button.setText("⏹️ 正在听...")
            self.voice_button.setChecked(True)
        else:
            self.voice_button.setText("🎤 语音")
            self.voice_button.setChecked(False)

    # 🟢 [修改] 接收到耳朵识别出的文字后，自动发送
    def handle_voice_input(self, text):
        stop_speaking()  # 🟢 [新增] 一旦听到主人说话，立刻打断AI正在说的语音！
    ...
    def send_message(self):
        ...
        """
        stop_speaking()  # 🟢 [新增] 如果你手动点击发送或按回车，也立刻让AI闭嘴！
    ...
    def on_ai_response_ready(self, response):
        ...
        """
        # 🟢 [新增] 针对完整的一大段话，强行洗成简体中文
        if isinstance(response, str):
            response = zhconv.convert(response, 'zh-cn')
        ...
        if self.loading_bubble is not None:
            ...
            if not self.ai_controller.ai_response_thread.use_streaming:
                ...
                # 🟢 [新增] 非流式回答发声 (放入新线程防止卡死界面)
                if hasattr(self, 'voice_thread') and self.voice_thread.is_voice_mode_on:
        ...
        if self.ai_controller and self.ai_controller.ai_response_thread.use_streaming:
            if self._stream_buffer.strip():
                ...
                # 🟢 [新增] 流式回答结束时，读取缓冲区内容并发声
                if hasattr(self, 'voice_thread') and self.voice_thread.is_voice_mode_on:
                    threading.Thread(target=speak, args=(self._stream_buffer,), daemon=True).start()
